  ...
  return (
    <>
      <DashboardLayout
        ...
        sidebar={<ProjectSidebar projects={myProjects} activeView={activeView} onSelectView={selectView} />}
        main={
            ...
            {activeView === "home" ? (
              <>
                ...
                <div className="grid min-h-[460px] grid-cols-1 gap-y-8 divide-y divide-white/10 md:grid-cols-[1fr_2fr] md:gap-y-0 md:divide-x md:divide-y-0">
                  ...
                  <WorkStationPanel tasks={tasks} onTasksChange={setTasks} currentUserId={currentUser.id} myProjects={myProjects} />
                </div>
              </>
            ) : (
              <ProjectContent key={activeView} projectId={activeView} currentUser={currentUser} initialTaskId={deepLinkTaskId} />
            )}
          </div>
        }
        rightPanel={
