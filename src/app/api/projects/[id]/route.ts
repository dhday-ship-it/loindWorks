...
export async function GET(
  ...
  const project = await prisma.project.findUnique({
    ...
    include: {
      ...
      files: {
        where: { requestId: null, logId: null, taskId: null },
        orderBy: { createdAt: "desc" },
        include: { uploader: { select: { id: true, name: true, email: true } } },
      },
    },
...
export async function PATCH(
  ...
  if (phases !== undefined) {
    if (!Array.isArray(phases) || phases.length === 0) {
      return NextResponse.json({ error: "최소 1개 이상의 단계가 필요합니다." }, { status: 400 });
    }
    data.phases = phases;
  }

  const phaseCount = data.phases ? data.phases.length : (existing.phases as string[]).length;

  if (currentPhase !== undefined) {
    if (typeof currentPhase !== "number" || currentPhase < 0 || currentPhase > phaseCount - 1) {
  ...
  if (Array.isArray(memberUserIds)) {
    await prisma.projectMember.deleteMany({ where: { projectId: id, roleLabel: "팀원" } });
    if (memberUserIds.length > 0) {
      await prisma.projectMember.createMany({
        data: memberUserIds.map((userId: string) => ({ projectId: id, userId, roleLabel: "팀원" })),
        skipDuplicates: true,
      });
    }
  }
